...
//	}
	...
	__HD__
	RGB InterpolateColor(const RGB *colors, const float b0, const float b1, const float b2) const {
		return b0 * colors[v[0]] + b1 * colors[v[1]] + b2 * colors[v[2]];
	}
	__HD__
	RGB InterpolateColor(const RGB *colors, const float b1, const float b2) const {
		const float b0 = 1.f - b1 - b2;
		return b0 * colors[v[0]] + b1 * colors[v[1]] + b2 * colors[v[2]];
	}
